...
#define   KPM_FREE(ptr)    if(ptr) {free(ptr);(ptr)=NULL;}
#endif
...
#pragma pack(pop)
  ...
  struct TKPMArchive
  {
    ...
    TKPMArchive* CreateClone(DWORD& nError)
    {
      nError = ERROR_SUCCESS;

      TKPMArchive* pArchive = NULL;
      
      if((pArchive = KPM_ALLOC(TKPMArchive, 1)) == NULL)
      {
      ...
      {
        *pArchive = *this;
        pArchive->BlockCache.pLastFile = NULL;
        pArchive->BlockCache.BufferOffSet = 0;
        pArchive->BlockCache.dwblockOffSet=0;
        //内存泄露
        pArchive->BlockCache.pbBuffer = KPM_ALLOC(BYTE, pArchive->dwBlockSize);
        if(pArchive->BlockCache.pbBuffer ==NULL)
        {
          nError = ERROR_NOT_ENOUGH_MEMORY;
          KPM_FREE(pArchive);
        }
    ...
    BOOL DestoryClone(TKPMArchive* pArchive)
    {
      if(pArchive)
      {
        KPM_FREE(pArchive->BlockCache.pbBuffer);
        KPM_FREE(pArchive);
        return TRUE;
      }
      return FALSE;
