...
bool Query::init(Manager *manager) {
  if (nullptr == manager) return false;
  manager_ = manager;
  isready_ = true;
  return true;
}
...
bool Query::query() {
  ...
  bool result = manager_->query(sql_);
  return result;
}
...
bool Query::fetcharray(db_fetch_array_t &db_fetch_array) {
  ...
  do {
    for (i = 0; i < columncount; ++i) {
      ...
      auto columntype = manager_->gettype(i);
...
bool Query::fetch(char *str, size_t size) {
  ...
  for (i = 0; i < columncount; ++i) {
    const char *columnname = manager_->get_columnname(i);
    auto columntype = static_cast<int8_t>(manager_->gettype(i));
    sstream << columnname;
    sstream << columntype;
  }
  int32_t row = 0;
  ...
  do {
    for (i = 0; i < columncount; ++i) {
      type::variable_t value = manager_->get_data(i, "");
      auto columntype = manager_->gettype(i);
      if (kDBColumnTypeString == columntype) {
        sstream << value.c_str();
      } else if (kDBColumnTypeNumber == columntype) {
