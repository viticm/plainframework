...
bool Interface::remove(int16_t id) {
  ...
  connection = pool_->get(connection_idset_[size_ - 1]);
  if (nullptr == connection) {
    Assert(false);
    return false;
  }
  connection_idset_[managerid] = connection_idset_[size_ - 1];
  connection_idset_[size_ - 1] = ID_INVALID;
